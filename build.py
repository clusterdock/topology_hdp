...
def main(args):
    ...
    # Higher version of Java (since JDK 1.8.0_161) installed so as we can get unlimited JCE policy
    java_home_base_dir = '/usr/jdk64'
    jdk8_minor_version = '212'
    # Auth param is not constant - it keeps generated by OTN. So login with OTN account and the auth param
    # is found in the URL of JDK download (https://www.oracle.com/technetwork/java/javase/downloads/jdk8-downloads-2133151.html)
    java_download_auth_param = '1561015170_c295a88735ba221d7748bfe9cd48a865'
    java_sha256_sum = '3160c50aa8d8e081c8c7fe0f859ea452922eca5d2ae8f8ef22011ae87e6fedfb'
    java_install_commands = [
        'mkdir -p {}'.format(java_home_base_dir),
        ('curl -jkLH "Cookie: oraclelicense=accept-securebackup-cookie" -o /tmp/java.tar.gz '
         'https://download.oracle.com/otn/java/jdk/8u{version}-b10/59066701cf1a433da9770636fbc4c9aa/'
         'jdk-8u{version}-linux-x64.tar.gz?AuthParam={auth}').format(version=jdk8_minor_version,
                                                                     auth=java_download_auth_param),
         'echo "{}  /tmp/java.tar.gz" | sha256sum -c -'.format(java_sha256_sum),
         'gunzip -c /tmp/java.tar.gz | tar -xf - -C {}'.format(java_home_base_dir),
         'rm -f /tmp/java.tar.gz'
    ]
    for node in cluster:
        node.execute(' && '.join(java_install_commands), quiet=quiet)
    ...
    if hdp_version_tuple >= (3, 0, 0, 0):
        primary_node.execute('ambari-server setup -v -s --java-home={}/jdk1.8.0_{}'.format(java_home_base_dir,
                                                                                           jdk8_minor_version),
                             quiet=quiet)
    else:
